...
void Detect() {
	...
	if(out.channels() == 1)
		...
		inRange(out, Scalar(minMax1.first, minMax2.first, minMax3.first), Scalar(minMax1.second, minMax2.second, minMax3.second), grayImage);
...
void on_conversion_change(int v, void *ptr) {
	setTrackbarPos("Conversion code", binaryWindow, v);
	code = codes[v];
	Detect();
}
...
void CreateTrackbars() {
	createTrackbar("Conversion code", binaryWindow, &code, codes.size() - 1, on_conversion_change);
