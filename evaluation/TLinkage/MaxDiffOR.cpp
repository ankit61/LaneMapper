...
namespace LD {
	void MaxDiffOR::operator()(const ArrayXf& _clusters, const std::unordered_map<int, vector<ulli> >& _clusterID2PtIndices, ArrayXf& _out, const int& _noiseIndex) {
		...
		for(ulli i = 0; i < count.size() - 2; i++) {  //intentionally skipping last value; FIXME: may be parallelized; 	
