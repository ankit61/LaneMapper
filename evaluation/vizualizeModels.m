...
while(ischar(lineM))
    ...
    for i = 1:numModels
		...
		if(strcmpi(model, 'surface'))
			Z = params(1) + params(2) * X + params(3) * Y + params(4) * (X .^ 2) + params(5) * (X .* Y) + params(6) * (Y .^ 2) + params(7) * (X .^ 3) + params(8) * ((X .^ 2) .* Y) + params(9) * (X .* (Y .^ 2)) + params(10) * (Y .^ 3);
		elseif(strcmpi(model, 'plane'))
			Z = -(params(1) * X + params(2) * Y + params(4)) / params(3);
		elseif(strcmpi(model, 'road'))
			Z = params(1) + params(2) * X + params(3) * Y + params(4) * (X .^ 2) + params(5) * (Y .^ 2);
		elseif(strcmpi(model, 'line3D'))
			t = [-10:0.1:10]';
			X = params(1) + params(4) * t;
			Y = params(2) + params(5) * t;
			Z = params(3) + params(6) * t;
        elseif(strcmpi(model, 'line2D'))
            t = [-10:0.1:10]';
            x = 10;
			...
			Y = y - params(1) * t;
			Z = -1.68 + 0 * t;
        elseif(strcmpi(model, 'spline'))
            splineParams = 6;
            ycfs = reshape(params(2:params(1)), [splineParams, (params(1) - 1) / splineParams])';
            zcfs = reshape(params((params(1) + 1):end), [splineParams, (params(1) - 1) / splineParams])';
            X = min(ycfs(:,1)):0.1:max(ycfs(:,2));
            Y = zeros(size(X));
            Z = zeros(size(X));
            for i=1:length(X)
                for r=1:size(ycfs, 1)
                    t = X(i) - ycfs(r,1);
                    if((X(i) >= ycfs(r, 1) && X(i) < ycfs(r, 2)))
        ...
        color = [0,1,0];
        for j = 1:stride:size(velo_img, 1)
            plot(velo_img(j, 1), velo_img(j, 2),'o', 'MarkerFaceColor', color, 'MarkerEdgeColor', color);
        end
        
        fgetl(fM);
		fgetl(fM);
    end
    saveas(fig, fullfile(outputDir, [outputPrefix lineM]));
