...
namespace LD {
	void SurfaceDataMaker::ParseXML() {
		...
		if(m_segRoot.empty() || m_segImgPrefix.empty() || m_outputFilePrefix.empty() || (m_saveVizImg && m_vizImgPrefix.empty()) || !m_minPoints)
	...
	void SurfaceDataMaker::ProcessProjectedLidarPts(const Eigen::MatrixXf& _veloImg, const Mat& _veloPoints, const Mat& _reflectivity, Mat& _inputImg) {
		...
		string outFile = m_outputRoot + "/" + m_outputFilePrefix + m_imgBaseName.substr(0, m_imgBaseName.size() - 3) + "txt";
		...
		for(int i = 0; i < _veloImg.rows(); i++) {
			...
			if(isValid(y, x, segImg.rows, segImg.cols) && segImg.at<unsigned char>(y, x)) {
				if(m_printProjectedPts)
					...
					fout << _veloPoints.at<float>(i, 0) << "\t" << _veloPoints.at<float>(i, 1) << "\t" << _veloPoints.at<float>(i, 2) << endl;
