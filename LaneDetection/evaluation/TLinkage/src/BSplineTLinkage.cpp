...
namespace LD {
	...
	void BSplineTLinkage::CreateSplineInterpolants(ArrayXf _model, alglib::spline1dinterpolant& _yOfX, alglib::spline1dinterpolant& _zOfX) {
		...
		y(i + 1)  = _model(2 + ySegmentStart) + t * (_model(3 + ySegmentStart) + t * (_model(4 + ySegmentStart) + t * _model(5 + ySegmentStart)));
		z(i + 1)  = _model(2 + zSegmentStart) + t * (_model(3 + zSegmentStart) + t * (_model(4 + zSegmentStart) + t * _model(5 + zSegmentStart)));
